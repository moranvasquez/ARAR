...
plot(resid_ti,xaxt="n",yaxt="n",xlab="",ylab="",pch=19)
...
eje.x <- round(quantile(1:n, probs = c(0, 0.5, 1)), 0)
eje.y <- round(seq(min(Di), max(lim.max), length.out = 3), 3)
par(mfrow=c(1,1),pty="s",mex=0.4)
par(mar=c(10,10,10,5))
...
cat(paste0(porc.puntos.fuera, "% se salen de la envolvente simulada\n"))
